...
void print_reverse_stack(struct stack *stack) {
  ...
  // reverse the stack
  //while (next_card) {
  //    struct stack *current_card = next_card;
  //    next_card = next_card->next;
      // make sure the first card in the stack doesn't create a cycle
  //    if (last_card->next == current_card) { last_card->next = 0x0;  }
  //    current_card->next = last_card;
  //    last_card = current_card;
  //}

  // print the stack
  for (struct stack *i = last_card; i; i = i->next) {
    draw_card(i->card);
  }
}
