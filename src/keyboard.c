        ...
        if (destination) {
          ...
          if (_marked_cards_count > 1 &&
            ...
            if (valid_move(*origin, *destination)) {
              ...
              //if (foundation_stack(*destination)) {
                // make sure foundation card has a dummy next value
  ...
  if (term_size_ok()) {
    ...
    case KEY_SPACEBAR:
      if (cursor_on_stock(cursor)) {
        ...
        struct card *tmp;
          if (tmp = stack_pop(&(deck->stock))) {
            int destination_y = (deck->waste_pile)->card->frame->begin_y;
            int destination_x = (deck->waste_pile)->card->frame->begin_x;
            if (!stack_empty(deck->waste_pile) && maneuvre_stack(deck->waste_pile)) {
              destination_y++;
            }
            stack_push(&(deck->waste_pile), tmp);
            frame_set((deck->waste_pile)->card->frame, destination_y, destination_x);
          }
          if (stack_empty(deck->stock)) {
